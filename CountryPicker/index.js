...
const countryCodeInput = document.getElementById("AddRegion");
const selectCountry = document.getElementById("SelectCountry");

function insertCountryOptions(countriesList = countries) {
  for (country of countriesList) {
    const option = `
		<li class="optionCountry" data-code="${country.code}" onclick="handleOptionClick(this)">
...
insertCountryOptions();
const options = selectCountry.querySelectorAll(".optionCountry");

function handleInputChange(e) {
  if (e.target.value.length > 0) {
    selectCountry.classList.add("open");
    // const filteredCountries = countries.filter((country) =>
    //   country.name.toLowerCase().includes(e.target.value.toLowerCase())
    // );
    // console.log(filteredCountries);
    // insertCountryOptions(filteredCountries);
    options.forEach((option) => {
      let isMatched = option.innerText
        .toLowerCase()
...
function handleOptionClick(option) {
  ...
  countryInput.value = option.innerText;
